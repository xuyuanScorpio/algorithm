...
public class LeetCodeAVL {
    ...
    public HashMap<TreeNode, Integer> getHeightMap() {
        return heightMap;
    }
    ...
    private TreeNode rotateRight(TreeNode p){
        ...
        heightMap.put(left, Math.max(getHeight(p.left), getHeight(p.right)) + 1);
        return left;
    }
    ...
    private void fixAfterInsertion(int key){
        ...
        while(!stack.isEmpty()){
            ...
            if(Math.abs(d) <= 1){
                ...
                if(d == 2){
                    if(key - p.left.value < 0){
                        p = rotateRight(p);
                    }else {
                        p = firstLeftThenRight(p);
                    }
                }else {
                    if(key - p.right.value > 0){
                        p = rotateLeft(p);
                    }else {
    ...
    public class ConvertSortedArrayToBinarySearchTree{
        ...
        public TreeNode soredArrayToBST_UsingAVL(int[] nums){
